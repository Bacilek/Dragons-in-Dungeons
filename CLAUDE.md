...
**Combat rolls**: attack roll = d20 + STR mod + weapon.bonus_damage vs target `armor_class`. Player AC = 10 + DEX mod + armor item `bonus_ac` (recalculated by `GameState.recalculate_stats()`). Enemy AC = type `"ac"` field + floor/5. Enemy attack roll = d20 + floor/3 vs player AC. **Critical hit (nat 20)**: auto-hit + 2× damage + gold "CRITICAL HIT!" overlay + screen shake. **Fumble (nat 1)**: always miss + red "CRITICAL FAIL!" overlay + mild shake. Both overlay and shake via `GameState.crit_banner` and `GameState.screen_shake` signals; HUD and Player.gd listen.
...
Drinking any POTION spawns an `Empty Bottle` (TOOL type, `sprites/items/Materials/BottleSmall.png`) at player's feet. Walking onto a WATER tile with an Empty Bottle in inventory auto-fills it → `Bottle of Water` (FOOD, heal=60 hunger, BottleMedium sprite). Walking onto MUD → `Bottle of Mud` (TOOL, future use). Fill logic in `player.gd _try_fill_bottle()`, called after every move step.
...
`_doors: Dictionary` maps `Vector2i → {is_open: bool, locked: bool, sprite: Sprite2D, tex_open, tex_closed}`. Auto-opens when entity steps on tile, auto-closes when entity leaves. Enemies open doors and walk through in same turn. **Locked doors**: enemies cannot open (blocked by `is_walkable_for_enemy` + `open_door` guard); player auto-unlocks by walking through (free action). Purple sprite tint = locked. F key on closed+unlocked door with Thief Tools → `_attempt_lock_door()` (DC 10 DEX); fail consumes tools. F key on locked door → unlock+open (action). Functions: `has_door_at`, `is_door_open`, `is_door_locked`, `open_door`, `close_door`, `lock_door`, `unlock_door`.
