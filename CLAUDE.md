...
- **Quickbar/inventory hover tooltips**: `hud.gd` has `_qbar_tooltip` Panel (follows mouse); `inventory_overlay.gd` has `_inv_tooltip` Panel. Both show item name, weapon die + damage_type + `range: N tile(s)` + Two-handed + `[url=keyword:heavy]Heavy[/url]` lines, heal dice, and description. Quickbar tooltip also handles ability bar mode (shows ability name + description). **Keyword glossary popup**: `KEYWORD_GLOSSARY` dict maps keyword → explanation text. Hovering a `[url=keyword:X]` link in either tooltip shows a secondary mini-panel beside the main tooltip. Both hud.gd (`_glossary_popup`) and inventory_overlay.gd (`_inv_glossary_popup`) implement this independently with identical KEYWORD_GLOSSARY content. **Inventory tooltip Ctrl-freeze**: in `inventory_overlay.gd`, pressing Ctrl while `_inv_tooltip` is visible freezes it in place and sets `_inv_tooltip_rtl.mouse_filter = MOUSE_FILTER_PASS` so `meta_hover_started` fires for keyword links. Ctrl again or closing inventory calls `_unfreeze_tooltip()` which restores `MOUSE_FILTER_IGNORE`.
