...
- **`SaveManager`** (`scripts/autoloads/save_manager.gd`) — single-slot run save at `user://save/run.json` (atomic write + `.bak` backup, deleted on death/win). Full Phase-A serialization is live (`GameState.to_dict()/from_dict()`, hand-written `Stats`/`Item` dict pairs, abilities rebuilt via talent replay) — only the Continue-flow UI (session 3c) remains. Details: `scripts/autoloads/CLAUDE.md`, design: `docs/architecture/SAVE_LOAD_ARCHITECTURE.md`.
