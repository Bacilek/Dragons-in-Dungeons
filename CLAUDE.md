...
- **Chat log tooltips**: hover `[url=meta]...[/url]` tags → `hud.gd._format_tooltip(meta)`, formatter bodies in `scripts/ui/tooltip_formatters.gd`. **RULE: every new damage source must get a `[url=kind:key=val,...]` tag on the number** — never log bare damage numbers. For the player-damage (`dmg`) tooltip specifically, a new bonus source does **NOT** need a `tooltip_formatters.gd` edit — see the generic bonus-source mechanism below.
- **RULE — damage stacking**: when multiple bonus damage sources apply to the same attack, sum them into the base damage **before** calling `Stats.take_damage()`/`DungeonFloor.show_damage()` — one call, one floater, one chat log line. Player-damage bonus sources (Rage, Frenzy, Ironwood Bark, Divine Fury, and any future one) are appended as `{label, color, value}` dicts to a local `bonus_sources`/`extra_sources` array and encoded via `CombatMath.encode_bonus_sources()` into the `dmg` meta string's single `extra=` field; `TooltipFormatters.fmt_dmg_tooltip()` decodes and renders that list generically. **Adding a new player damage modifier only requires appending one entry to that array at the point the bonus is computed — no separate tooltip_formatters.gd edit.** The visible log line itself never lists per-source names or amounts, only the combined number. See `scripts/entities/CLAUDE.md` for the full rule and `player.gd._bump_attack()`/`PlayerRanged.ranged_attack()` for the reference implementation.
