...
**Barbarian Tier 2 — Berserker** (levels 7–12, gated behind Necromancer kill on floor 10):
- Level-point schedule: levels 1–5 grant Tier 1 points immediately. Level 6 grants nothing. Levels 7–12 grant Tier 2 points — if `tier2_unlocked == false` at level-up, points queue in `_pending_tier2_points` and are released all at once by `unlock_tier2()` when the Necromancer dies. Levels 13+ grant nothing until Tier 3.
- `GameState.tier2_unlocked: bool` — set in `unlock_tier2()`, called from `player.gd._finish_kill()` when `killed_name == "Necromancer"` and class is Barbarian. `_setup_barbarian_tier2_talents()` appends 3 `Talent` objects to `_class_talents`; pending points are released immediately.
...
**Berserker Tier 2 talents** (unlocked on Necromancer kill, max 3 ranks each):
