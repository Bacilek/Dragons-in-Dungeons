...
- **`_interact_action(target)`**: when `target != Vector2i(-1, -1)` (called from RMB), only check that exact tile for traps/doors — NOT all 8 neighbors.
