...
- **Rage** (ability_id `"rage"`) — in slot 0. **Baseline**: 1 use/floor (long rest). 10-turn countdown; no DR at rank 0. +2 damage on STR melee attacks. Red sprite tint. Rage ends if heavy armor equipped (`item.is_heavy_armor`). Activation is a **free action**. Talent rank 1+: countdown pauses when player attacked or was hit last turn. Rage talent upgrades (via `invest_talent("rage")`): R1 +1 use (→2), R2 +1 use (→3) + 25% physical DR (Slashing/Piercing/Bludgeoning only), R3 +1 use (→4) + 50% physical DR. DR applied in `take_damage_raw(amount, ignore_rage, damage_type: String)` — status and trap damage pass `""` and bypass DR.

**Barbarian Tier 1 talents** (no fixed level-up unlocks — all are point-gated):
- **Rage** (`talent_id: "rage"`, max 3): See above. Rage ability already in bar at game start.
