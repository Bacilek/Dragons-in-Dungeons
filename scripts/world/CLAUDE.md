...
**Touch/teleport/no-shape TILE spells and touch-range SELF spells** (Misty Step, Mage Armor, Invisibility): `show_touch_target_preview(tile: Vector2i, in_range: bool = true)` — a plain purple highlight on the exact tile the spell would actually resolve at, with NO enemy requirement at all (unlike `show_single_target_preview()` above). For a no-shape `TargetKind.TILE` spell (Misty Step, Light), the highlighted tile is whatever's currently hovered, gated on `in_range`. For a touch-range (`range_tiles > 0`) `TargetKind.SELF` spell, the highlighted tile is always the CASTER's OWN tile regardless of where the mouse is — any click confirms a self-cast for these (see `scripts/entities/CLAUDE.md`'s "Wizard leveled spells" → Mage Armor), so the preview shows what will actually happen ("this lands on you") rather than following the cursor. **Bugfix**: both of these previously fell through `_update_spell_aoe_preview()`'s dispatch to the generic `else: hide_aoe_preview()` branch and got no preview at all — a player arming Misty Step had no visual indication of where they'd actually teleport to. `Spell.TargetKind.SELF` spells with `range_tiles <= 0` (Shield) instant-cast on activation and never reach this dispatch armed, so they need no branch.
...
**Named-ammo loot weighting** (owner request): every uniform "pick a random eligible `ITEM_POOL`
...
and `_spawn_shop()`'s stock roll are unaffected — different, purpose-built selection shapes, not
this uniform-pick pattern.
