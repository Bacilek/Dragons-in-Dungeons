...
- **`SpellDb`** (static factory, `RefCounted`) — `get_spell(id) -> Spell` builds all spells in
  ...
  `["fog_cloud", "pass_without_trace", "cure_wounds", "aid"]` — `fog_cloud` is the only
  ...
  `pass_without_trace`/`cure_wounds`/`aid` are all Ranger-exclusive entries (`class_list =
  ["RANGER"]` only, never added to `LEVELED_SPELL_IDS`) — see `scripts/entities/CLAUDE.md`'s
  "Ranger class" for Cure Wounds'/Aid's full mechanisms)
