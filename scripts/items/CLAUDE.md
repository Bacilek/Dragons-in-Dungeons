...
| `is_loading` / `loading_used_this_turn` | bool / bool | Loading weapon (currently Heavy Crossbow, Longbow): can fire at most once per real player turn, unconditionally — no other effect/ability/talent can force a second shot with it, even a hypothetical future multi-attack source. `loading_used_this_turn` is the live per-weapon cooldown flag, set the instant a shot actually fires (`PlayerRanged.ranged_attack()`/`ranged_attack_tile()`, right after `show_projectile()` — the point of no return) and checked at the very top of both functions BEFORE anything else happens (no animation, no ammo consumed, no turn spent if blocked — just a gray "needs to be reloaded" log line). Reset every real player turn in `player.gd._on_turn_started()`'s `if not came_from_revert:` block, scoped to whichever weapon is currently in the `"ranged"` equipment slot — a Loading weapon sitting unequipped in the bag was never able to fire in the first place, so nothing needs resetting there. Deliberately NOT meaningfully serialized (`is_loading` is, `loading_used_this_turn` isn't — always loads back to `false`, same "combat-transient, safe default" precedent as several `Stats` fields in `scripts/entities/CLAUDE.md`). Shown as a hoverable "Loading" keyword in weapon tooltips |
...
3. **Damage + type** — `NdM[+bonus] [color=gray]Type[/color]`, `NdM` recovered from the stored
   flat `damage_die_min`/`damage_die_max` range via `CombatMath.dice_notation()` (so Greatsword/Maul
   show `2d6`, not `1d12`), plus `bonus_damage`/`damage_type`. (No current weapon has more than one
   damage-type instance; a future one would be `+`-joined here — `build()`'s own comment marks where.)
...
## Thrown weapons
**Poisoned Arrow** (`Item.item_name == "Poisoned Arrow"`, Simple, Piercing, Finesse+Light,
`range=4`, `uses_max=1` — single-use, unlike every other thrown weapon's 5 charges): not a floor-
loot item (`fmin`/`fmax=99` sentinel, same convention as Mold) — its only source is looting a
sprung-or-unsprung Tripwire trap's hidden dispenser, see `scripts/world/CLAUDE.md`'s "Tripwire
trap". A landed hit ALSO applies the real Poisoned condition (`"poisoned_condition"`, 6 turns) —
`PlayerThrowTool._throw_weapon()`'s Sap-mastery-style `item.item_name == "Poisoned Arrow"` check,
right alongside the Sap mastery hook, since this is a name-specific effect rather than a mastery.
...
| Heavy Crossbow | +0 | 5 | 20 | Bolt | DEX | Martial | Push |
| Longbow | +0 | 8 | 30 | Arrow | DEX | Martial | Slow |
| Pistol | +0 | 2 | 5 | Bullet | DEX | Martial | Vex |
| Musket | +0 | 2 | 6 | Bullet | DEX | Martial | Slow |

**Pistol/Musket** are both also `is_loading=true` (fires at most once per real turn, no exceptions
— see `scripts/items/CLAUDE.md`'s `is_loading` field entry above); Musket is additionally
`is_two_handed=true`. Neither is `is_heavy` — unlike Heavy Crossbow/Longbow, a low-DEX character
takes no Disadvantage penalty wielding either firearm. Their short 2-tile normal range (versus the
bow family's 4-8) is deliberate: a firearm's real-world short effective range, reined in hard by
this project's own `/20`-ceil ranged-distance convention (`scripts/entities/CLAUDE.md`'s "Ranged
distance scaling convention") — Pistol/Musket are the first ranged weapons whose normal range sits
below the bow family's floor rather than following the same scaling the rest of the roster does.

Heavy Crossbow and Longbow are both also `is_heavy=true` (DEX 13+ or Disadvantage), `is_two_handed=true` (cosmetic for a ranged weapon — see root `CLAUDE.md`'s note that `is_two_handed` doesn't block the ranged slot), and `is_loading=true` (fires at most once per real turn, no exceptions — see the `is_loading` field entry above). Ranged weapons' `range` follows the `/20`-ceil rule (`scripts/entities/CLAUDE.md`'s "Ranged distance scaling convention"); `long_range` is hand-tuned per weapon, not a fixed multiplier of `range` anymore (owner-authored values, not derived).

## Ammo items
`Item.ammo_item_name` on a ranged weapon names a separate stackable `Item` (`Item.Type.TOOL`, no combat stats of its own — currently **Arrow** for the Short Bow and Longbow, `sprites/items/ammo/arrow.png`, **Bolt** for the Heavy Crossbow, `sprites/items/ammo/arrow_gold.png` — a distinct color tier reused as a stand-in bolt sprite since no dedicated bolt art exists yet — and **Bullet** (renamed from "Buckshot" — same item, same `sprites/items/ammo/bullet.png` art, just a more accurate firearm-ammo name), consumed by the **Pistol** and **Musket** — see "Weapons (current, game-wide)" below) consumed 1-per-shot. The flying-projectile VFX during a ranged attack is separate and still uses `sprites/weapons/weapon_arrow.png` (`PlayerRanged.ARROW_SPRITE`) regardless of which ammo item was consumed. Found/looked-up by `item_name` match across the quickbar then bag (`PlayerAmmo.find_ammo_stack()`/`remove_ammo_stack()` in `scripts/entities/player_ammo.gd`) — a weapon with `ammo_item_name == ""` falls back to the legacy `consumes_on_ranged` pattern (decrements the weapon's own `quantity`, e.g. old Throwing Daggers) or fires with infinite ammo.
...
The only weapons in the game are the Barbarian's starting **Greataxe** (melee, two-handed, given via `GameState._give_barbarian_starting_items()` — never spawns as floor loot), **Short Bow**, **Heavy Crossbow** above (formerly named "Crossbow" — renamed as the first of a small family of ranged weapons sharing the same normal-range/FOV-long-range rule; 1d10 Piercing, Martial, requires **Bolt** ammo), **Longbow** (ranged, floor loot, 1d8 Piercing, Martial, `is_heavy=true`, `is_two_handed=true`, `weapon_mastery="Slow"`, normal range 8 (further than Short Bow's 4 / Heavy Crossbow's 5), requires **Arrow** ammo (shared with Short Bow); floor loot `fmin`/`fmax` 5–10, sprite `weapons/bow.png` — see "Weapon masteries" above for what Slow does), **Rapier** (melee, 1d8 Piercing, Martial, `is_finesse=true` — attack/damage use `max(STR, DEX)` — `weapon_mastery="Vex"`; not Light, not Two-handed; floor loot `fmin`/`fmax` 1–10, `weapon_arrow.png`-free sprite `weapon_duel_sword.png`), **Greatsword** (melee, floor loot, 2d6 Slashing — stored as the flat range `damage_die_min/max=2/12`, which `CombatMath.dice_notation()` recovers as a genuine 2d6 (`Rng.roll_dice(2, 6)`) for both the roll and the tooltip — Martial, `is_heavy=true`, `is_two_handed=true`, `weapon_mastery="Graze"`; floor loot `fmin`/`fmax` 3–10, sprite `weapon_knight_sword.png` — no dedicated greatsword sprite exists yet), **Glaive** (melee, floor loot, 1d10 Slashing, Martial, `is_heavy=true`, `is_two_handed=true`, `is_reach=true`, `weapon_mastery="Graze"`; floor loot `fmin`/`fmax` 3–10, sprite `weapon_spear.png` — no dedicated polearm sprite exists yet — the first and so far only weapon to set `is_reach`), **Maul** (melee, floor loot, 2d6 Bludgeoning, Martial, `is_heavy=true`, `is_two_handed=true`, `weapon_mastery="Topple"`; floor loot `fmin`/`fmax` 3–10, sprite `weapon_big_hammer.png` — the first weapon with Bludgeoning damage type), and **Quarterstaff** (melee, floor loot, Simple, Bludgeoning, `weapon_mastery="Topple"`; 1d6 one-handed / 1d8 two-handed — `is_versatile=true`, `damage_die_min/max=1/6`, `versatile_die_min/max=1/8` at rest; floor loot `fmin`/`fmax` 1–10, sprite `weapon_green_magic_staff.png` — no dedicated plain-staff sprite exists yet — the first Versatile weapon, see "Versatile weapons" below), **Spear** (melee, floor loot, Simple, Piercing, `weapon_mastery="Sap"`; also Versatile 1d6/1d8 like the Quarterstaff, plus `is_thrown=true`, `range=2` (normal throw range, FOV beyond that at Disadvantage), `long_range=6`, `uses_max=5`; floor loot `fmin`/`fmax` 1–10, sprite `weapon_spear.png` (shared with Glaive — no dedicated javelin/spear-only sprite exists yet) — the first Thrown weapon, see "Thrown weapons" below), **Handaxe** (melee, floor loot, Simple, Slashing, 1d6, `weapon_mastery="Vex"`, `is_light=true` — the first Light weapon; also `is_thrown=true`, `range=2`, `long_range=6`, `uses_max=5` like the Spear; floor loot `fmin`/`fmax` 1–10, sprite `weapon_throwing_axe.png` — dual-wielding a second Light weapon in the Off-hand fires a bonus attack each melee swing, see "Dual-wielding" below), **Dagger** (melee, floor loot, Simple, Piercing, 1d4, `is_finesse=true`, `is_light=true`, `weapon_mastery="Nick"`; also `is_thrown=true`, `range=2`, `long_range=6`, `uses_max=5` like the Handaxe; floor loot `fmin`/`fmax` 1–10, sprite `weapon_knife.png` — see "Weapon masteries" above for what Nick does), **Scimitar** (melee, floor loot, Martial, Slashing, 1d6, `is_finesse=true`, `is_light=true`, `weapon_mastery="Nick"` — the second Nick-mastery weapon alongside Dagger, matching real 5e 2024's own pair; **not** Thrown, unlike Dagger/Handaxe — a plain melee-only Finesse+Light sword; floor loot `fmin`/`fmax` 1–10, 25gp, sprite `weapon_scimitar.png` (moved out of `sprites/weapons/_unused/`, formerly `weapon_katana.png`)), **Javelin** (melee, floor loot, Simple, Piercing, 1d6, `weapon_mastery="Slow"`; not Light, not Finesse, not Versatile — a plain Simple melee/thrown weapon; also `is_thrown=true`, `range=3`, `long_range=12`, `uses_max=5` like the other thrown weapons; floor loot `fmin`/`fmax` 1–10, sprite `weapon_spear.png` (shared with Spear/Glaive — no dedicated javelin sprite exists yet); 5sp — the cheapest weapon in the game. Distinct from the same-named `"thrown_weapon"` pool key on Orc Warrior/Ogre's own `ENEMY_POOL`/`BOSS_POOL` entries (`scripts/entities/CLAUDE.md`'s "Enemy D&D stat-block schema") — that's a separate enemy-attack mechanism building its own dropped `Item` from the enemy schema's own fields, unrelated to this `ITEM_POOL` entry, and the two happening to share a name is coincidental, not a shared code path), and **Torch** (melee, floor loot, Simple, Bludgeoning, 1d4, `is_torch=true`, **not** Light — equippable in Off-hand like a Shield without dual-wielding; also `is_thrown=true`, `range=2`, `long_range=4`, `uses_max=1` (a single use, fewer than every other thrown weapon); no mastery; floor loot `fmin`/`fmax` 1–10, sprite `weapon_torch.png` — no dedicated torch sprite exists yet, this is a placeholder path — see "Torch" above for the click-to-light mechanic, the melee/embedded-burn Fire mechanics, and the passive light bubble), **Pistol** (ranged, floor loot, 1d10 Piercing, Martial, `is_loading=true`, `weapon_mastery="Vex"`, normal range 2/long range 5, requires **Bullet** ammo; floor loot `fmin`/`fmax` 6–10, 250gp, sprite `weapons/bow_arrow_gold.png` — placeholder, no dedicated pistol sprite exists yet — the first firearm, and the first ranged weapon that is neither Heavy nor Two-handed), and **Musket** (ranged, floor loot, 1d12 Piercing, Martial, `is_two_handed=true`, `is_loading=true`, `weapon_mastery="Slow"`, normal range 2/long range 6, requires **Bullet** ammo (shared with Pistol); floor loot `fmin`/`fmax` 7–10, 500gp, sprite `weapons/bow.png` — placeholder, no dedicated musket sprite exists yet; the most expensive weapon in the game). All physical melee weapons that used to spawn as floor loot (Rusty/Short/Regular/Knight/Golden/Lavish Sword) and **Throwing Daggers** have been removed from `DungeonFloorData.ITEM_POOL`, `debug_panel.ALL_ITEMS`, and boss loot (`dungeon_floor.gd drop_boss_loot()`, now potions-only). Their sprite assets live under `res://sprites/weapons/_unused/` (unused, not deleted) in case they're reintroduced later.
...
`_bump_attack()`'s primary swing also deals a second, independent **2d4 + STR mod** Fire damage
instance (Off-hand/Cleave/Opportunity-Attack swings never get it — a documented scope limit,
matching the project's existing precedent of not extending every bonus to every attack path) — the
exact "one hit, two damage-type instances" shape `scripts/entities/CLAUDE.md`'s damage-stacking
rule documents (mirrors Zealot's Judgement Day). The STR mod is the same `dmg_mod` the primary
Bludgeoning instance already uses (a Torch is never Finesse/unarmed, so this is always the plain
STR modifier — folded into the Fire instance's own flat-mods array, shown as its own "STR mod"
line in the damage tooltip). **Unequipping** a lit Torch pauses nothing about its burn — only the
FOV/melee-Fire bonuses stop (both computed live off the equipment slots) — but a lit Torch lying on
the floor or embedded in an enemy instead grants its own passive light bubble, see below.
...
**Thrown** (`is_thrown=true`, `range=2`, `long_range=4`, `uses_max=1` — a single use, fewer than every other thrown weapon's 5):
...
damage is just the normal 1d4 Bludgeoning + STR — **no instant Fire damage on the throw itself**,
lit or not (this used to also deal a second 1d4 Fire instance on the hit; removed — the only Fire
this weapon deals now is the ongoing per-round burn below). Thrown at empty ground → lands as a
normal floor item (unchanged generic thrown behavior). Thrown at an enemy, on a miss or non-lethal
hit → embeds in `Enemy.embedded_items` (unchanged generic thrown-weapon embedding — no
Torch-specific code needed there at all), dropped at `grid_pos` when that enemy eventually dies
(`Enemy.die()`'s existing override). **A lit torch embedded in an enemy sets it burning for as
long as it stays lodged and lit** — `DungeonFloor.tick_torches()` (`scripts/world/CLAUDE.md`), on
top of ticking `torch_turns_remaining` down, rolls a fresh independent 2d4 Fire hit (no STR/ability
mod — this is a passive environmental DoT tick, not an attack roll) against that enemy each real
turn (same rate as a creature standing on a burning door — `DungeonFloor.
tick_fire_damage_for()`), with the usual floater/log line/kill handling. Ends when: the torch burns
out (`torch_turns_remaining` hits 0), the enemy dies, or the enemy steps into WATER — `Enemy.
_move_step()` douses any lit torch in its own `embedded_items` on a WATER tile exactly like it
already zeroes `stats.burning_turns` there (same "water extinguishes fire" rule, just for the
physically-embedded item instead of the DoT counter). A lit torch merely lying on the floor does
not damage anything standing on its tile — this is specifically about one still lodged in a living
target.

**Passive light bubble** (floor/embedded, distinct from the equipped +1 FOV bonus — **two
different radii**, not one shared value): `DungeonFloor._compute_torch_light_tiles()`, called from
`update_fog()` every fog recompute (no persistent light-source registry — see the design note
below), sweeps every `_floor_items` tile for a lit-and-unburnt Torch and unions a
`GameState.TORCH_LIGHT_RADIUS` (2) shadowcast into `_visible_tiles` around it; separately sweeps
every live enemy's `embedded_items` for one and unions a smaller `GameState.
TORCH_BURN_LIGHT_RADIUS` (1) shadowcast instead, centered on that enemy's **current** `grid_pos` —
a torch stuck in a moving, burning creature only lights the creature's own tile plus its immediate
neighbors, dimmer than one just sitting in the open on the floor. Either way the bubble moves/
persists with zero dedicated tracking state — an embedded one follows its enemy across turns for
free.
...
**Dragging onto (or unequipping into) any inventory slot**: `move_item()`'s `_find_matching_stack(item, exclude_src, exclude_idx)` scans the WHOLE quickbar+bag (not just the literal drop-target slot) for an existing pile the dragged/unequipped item `_items_can_stack()` with (see "Stackability rule" above); if one exists anywhere, the item merges into it (`_merge_into_stack()`) and the source slot is emptied, instead of landing wherever it was dropped. This covers both dragging one stack directly onto another AND dragging (or click-unequipping) a Thrown weapon out of an equipment slot into an arbitrary empty bag/quickbar slot — either way it finds and joins the existing pile rather than starting a second one, so two separate stacks of the same Thrown weapon can never end up sitting in different slots at once. A drop that finds no match still does the normal swap-into-that-slot.
...
2. **Requirements (1-2, no replacement)** — `Rng.range_i(1,2)` count, drawn from
   ...
   `ammo_item_name = Rng.pick(["Arrow","Bolt","Bullet"])`, `range = Rng.range_i(3,5)`,
...
## Major Arcana cards
22 new `ITEM_POOL`/`debug_panel.ALL_ITEMS` entries (`DungeonFloorData.MAJOR_ARCANA_NAMES` lists
every one by name), `Type.TOOL`, icon placeholder-reused from `Materials/plate/gold.png` (no
dedicated card art exists). Sentinel `fmin`/`fmax = 99` (same convention as Mold/Healing Herb)
keeps every one out of every generic floor-loot roll — the only spawn path is the Tenebrous NPC
prop's interaction (`scripts/world/CLAUDE.md`'s "Tenebrous prop"), which hands out exactly one
random entry from the list, once ever, per run. **Mechanically inert for now** — a plain `Type.TOOL`
item with no effect on use, occupying a bag slot purely as a collectible; what each card actually
does is a deliberately deferred design question, not a bug.
