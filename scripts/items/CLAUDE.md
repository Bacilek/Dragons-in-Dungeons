...
Heavy Crossbow and Longbow are both also `is_heavy=true` (DEX 13+ or Disadvantage) and `is_two_handed=true` (cosmetic for a ranged weapon — see root `CLAUDE.md`'s note that `is_two_handed` doesn't block the ranged slot). Ranged weapons' `range` follows the `/20`-ceil rule (`scripts/entities/CLAUDE.md`'s "Ranged distance scaling convention"); `long_range` is hand-tuned per weapon, not a fixed multiplier of `range` anymore (owner-authored values, not derived).
...
- **`SpellDb`** (static factory, `RefCounted`) — `get_spell(id) -> Spell` builds all spells in
  ...
  `["fog_cloud", "pass_without_trace", "cure_wounds", "aid", "barkskin"]` — `fog_cloud` is the only
  `LEVELED_SPELL_IDS` entry whose real 5e/5.5e class list actually includes Ranger (every other
  `LEVELED_SPELL_IDS` entry is Sorcerer/Wizard(/Warlock)-only on both rule sets, so it was
  deliberately NOT opened up to Ranger despite reusing the same shared spell pool);
  `pass_without_trace`/`cure_wounds`/`aid`/`barkskin` are all Ranger-exclusive entries (`class_list
  = ["RANGER"]` only, never added to `LEVELED_SPELL_IDS`) — see `scripts/entities/CLAUDE.md`'s
  "Ranger class" for Cure Wounds'/Aid's/Barkskin's full mechanisms)
