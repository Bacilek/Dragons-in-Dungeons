...
# 3 candidates rolled by GameState._roll_spell_learn_choices()). Modeled directly on
...
func _build_ui() -> void:
	...
	var hint := Label.new()
	hint.text = "Choose one spell to add to your spellbook. This choice is permanent."
	...
	var panel_h: float = y0 + choices.size() * (CARD_H + CARD_GAP) - CARD_GAP + 20.0
