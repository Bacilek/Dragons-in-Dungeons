...
# character reaches Tier 2's start level (TalentTiers.tier_start_level(2), level 8). Spawned by
# hud.gd on GameState.subclass_choice_required (emitted from GameState._check_tier2_level_gate()).
...
func _build_ui() -> void:
	...
	var hint := Label.new()
	hint.text = "You have reached level %d - pick one of the paths below. This choice is permanent." % TalentTiers.tier_start_level(2)
