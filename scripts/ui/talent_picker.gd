...
func _build_tier_section(tier: int, y: float) -> float:
	...
	if tier == 2 and not GameState.tier2_unlocked:
		...
		locked_lbl.text = "Tier 2  —  reach level %d to unlock" % TalentTiers.tier_start_level(2)
		...
		# Pending-points badge: Tier 2 points only pend while a Barbarian's subclass pick is open.
		var pending: int = GameState.talent_points.get(2, 0)
		if pending > 0:
			var badge := Label.new()
			badge.text = "%d point%s pending — choose your subclass" % [pending, "s" if pending != 1 else ""]
