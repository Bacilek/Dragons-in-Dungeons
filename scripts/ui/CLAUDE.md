...
Pixel Dungeon style: tier header with star bar (gray=spent / yellow=available / dark=locked) + icon row with dot rank indicators + bottom detail panel showing all rank descriptions + "Upgrade Talent ▲" button. **Tier 2 locked state**: while `not GameState.tier2_unlocked`, the Tier 2 section renders a gray "reach level 8 to unlock" row (`TalentTiers.tier_start_level(2)`), and if `GameState.talent_points[2] > 0` a right-aligned gold badge "N points pending — choose your subclass" (Tier 2 points only pend while a Barbarian's level-8 subclass pick is open).
...
**Grace period before hiding** (`_HOVER_CHAIN_HIDE_GRACE_SEC = 0.2`, `_hover_chain_outside_time`):
the per-frame check doesn't hide the instant the mouse leaves every rect in the chain — it only
hides once the mouse has been continuously outside the whole chain for longer than the grace
window, resetting to 0 the instant it re-enters any part of it. Without this, a single frame spent
crossing the small visual gap between a trigger (an icon/tile) and its tooltip box (which never
touches the trigger pixel-for-pixel) read as "left the chain" and closed the tooltip before the
mouse could ever reach it — reported as "can't hover into the tooltip on the first try," most
noticeable on the status tray's small tightly-packed icons. Every hover-chain implementation in
this directory carries its own copy of this same const/var pair (no shared base class — GDScript
composition-over-inheritance precedent, same reasoning as the split-out modules above):
`hud.gd`, `inventory_overlay.gd`, `mastery_picker.gd`, `cantrip_select.gd`,
`spell_learn_picker.gd`, `invocation_picker.gd`. Any new tile/icon hover-chain popup should copy
this pattern too, not the older instant-hide check.
