...
Slot type enforced via `_fits_slot()`: Main Hand (`"melee"`) rejects ranged items and vice versa; `"hand2"` (Off-hand) has no fits-slot case yet (like gloves/boots/head — a placeholder slot for a future dual-wield/offhand feature, not wired into combat). `equip()` always routes non-ranged weapons to `"melee"` regardless of how they're equipped (pickup, starting gear, debug give-item, or explicit equip) — Off-hand is never auto-populated.
