...
#
# Phase 1 of the migration (§8 step 1): output is byte-identical to the
# pre-refactor monolithic generator — same seeded rng, same call order.
...
static func generate(seed_val: int, floor_num: int) -> DungeonData:
	...
	# Initialize — what rooms exist. Phase 1: makes zero rng calls (see floor_planner.gd).
	var rooms: Array = FloorPlanner.plan(floor_num, "", rng)
	...
	# Paint — per-room paint (no-ops in Phase 1) + level-wide overlays.
	LevelPainter.paint(data, rooms, rng, "")
