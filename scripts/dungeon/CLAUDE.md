...
Spec: `docs/architecture/DUNGEON_GENERATION_ARCHITECTURE.md` + `docs/architecture/LOOP_BUILDER_ARCHITECTURE.md`. Phase 1 (pipeline split), Phase 2 (seeded-shuffle fix + Floor Feelings), and Phase 3 (LoopBuilder + room budget + retry/fallback orchestration) are done. Public entry point unchanged. The invariant is **reproducibility**: same seed+floor → identical output, no global-RNG dependence.
