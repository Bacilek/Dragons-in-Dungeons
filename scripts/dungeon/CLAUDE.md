...
`Room` extends `RefCounted` (deliberately NOT `Resource` — pure generation-time data). Fields: `type_id: String`, `rect: Rect2i` (empty until Build), `connections: Array[Room]`, `required: bool`. Virtuals: `min_size()`, `max_size()`, `max_connections()`, `min_connections()` (default 1), `paint(data, rng)` (no-op base). Concrete: `StandardRoom` (plain floor), `EntranceRoom`/`ExitRoom` (`required = true`, `min_connections() = 2` — see "Multi-entrance connectivity" below).

**TreasureRoom** (session 7c, implemented): dead-end vault (`min_size` 5x5, `max_size` 7x7, `max_connections() = 1`); `paint()` is still a no-op (guard only — a vault is plain floor). Runtime content — 3 guaranteed `ITEM_POOL` rolls, 1 guaranteed gold pile, the door lock, and floor≥4 traps — is `DungeonFloor._spawn_treasure(rect)` (`scripts/world/CLAUDE.md`).

**GardenRoom** (session 7d, implemented): inherits `StandardRoom`'s default sizing/`max_connections()` (pass-through, not gated); `paint(data, rng)` IS the content — carpets ~60% of interior FLOOR tiles with GRASS (per-tile `rng.randf() < 0.6`) then flood-fills one small WATER blob (4-9 tiles) from an anchor inset 1 tile off every edge (`rect.grow(-1)`), same flood-fill technique as `LevelPainter._place_water_mud()` but scoped to the room rect and using the per-room `rng` (this is now the first room `paint()` override that actually consumes rng calls — the old "all current room types are no-ops" comment in `level_painter.gd` is stale for any floor where a GardenRoom spawns). Herb items land separately via `DungeonFloor._spawn_garden_items(rect)`, which reads the GRASS tiles `paint()` already carved.

**BlacksmithRoom** (implemented, random-weapon-crafting feature — no standalone design doc):
inherits `TreasureRoom`'s sizing shape (5x5-7x7, `max_connections() = 1`), `paint()` a no-op guard
— content is runtime (`DungeonFloor._spawn_blacksmith(rect)`, `scripts/world/CLAUDE.md`).
**Not a `ROOM_POOL` Bernoulli entry** — unlike every other special room above, it's guaranteed
**exactly once, only on floor 4**: `FloorPlanner.plan()` special-cases
`if floor_num == 4: specials.append(BlacksmithRoom.new())` entirely outside the `ROOM_POOL` loop,
mirroring how the boss room is special-cased outside `ROOM_POOL` too — a `chance: 1.0` pool entry
