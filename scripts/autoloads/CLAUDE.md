...
`tier_for_level(lv)`/`tier_start_level(tier)`/`tier_unlocked(tier, tier2_unlocked, character_level)`
...
| `boss_defeated` | `boss_id: String` | any boss killed (player kill or Push-into-chasm) — no longer gates anything (Tier 2 is level-gated now); kept for future listeners |
...
**Tier 2 unlock + subclass selection (level-gated)**: Tier 2 unlocks by LEVEL alone — no boss kill (direct owner decision, 2026-09-10; `_on_boss_defeated()`/`TIER2_GATING_BOSS_ID` were removed). `gain_exp()` calls `_check_tier2_level_gate()` after every level-up: once `character_level >= TalentTiers.tier_start_level(2)` (8), a Barbarian (has subclasses) with `not subclass_chosen` emits `subclass_choice_required` — hud.gd spawns `scripts/ui/subclass_select.gd` (blocking, non-dismissable); its confirm calls `choose_subclass(name)` which sets `active_tier2_subclass` + `subclass_chosen = true` and calls `unlock_tier2()`. Other classes call `unlock_tier2()` directly. `unlock_tier2()` sets `tier2_unlocked` and — **Barbarian only** — runs `_setup_tier2_for_active_subclass()` + logs; for every other class it just flips the flag (bugfix: `active_tier2_subclass` defaults to "Berserker", so an unguarded setup grafted Berserker talents onto any class that reached the gate). `choose_subclass()` is one-time. Tier 2 points only pend in the brief window while a Barbarian's pick is open (the talent picker shows a pending badge).

**Tier scaffolding (Tiers 1–4)** — `docs/architecture/class-progression-rules.md` §4: `TIER_LEVEL_RANGES = {1: [1,7], 2: [8,13], 3: [14,19], 4: [20,20]}` → **6/6/6/1** points (a tier's count is its level-up TRANSITIONS, so T1 yields points on the way into levels 2-7; tier 4 is the Epic Boon pool, one point at 20). `gain_exp()` grants one point per crossed level into that level's own tier (bugfix: a multi-level XP grant used to award only one point). `Stats.MAX_LEVEL` (20) is a soft cap — `Stats.gain_exp()` stops leveling there, XP still accumulates. `tier_unlocked(tier) -> bool`: 1 = always; 2 = `tier2_unlocked`; 3 = level ≥ 14; 4 = level ≥ 20 (all level-only, the old `tier3_selected_class` multiclass stub is gone). `can_invest_talent()` gates on `tier_unlocked(t.tier)` plus `talent_points[t.tier] > 0`. Tier 3/4 CONTENT is not authored yet — points there accumulate with nothing to spend them on.
