...
**Weapon mastery ownership**: `Stats.known_weapon_masteries: Array[String]` (default empty) + `Stats.knows_mastery(name) -> bool`. A weapon's `Item.weapon_mastery` only triggers its effect if the wielder knows that mastery — nothing currently grants entries to this array, so mastery weapons (Greataxe/Cleave, Short Bow/Vex) have no mastery effect yet. See `scripts/items/CLAUDE.md`'s "Weapon masteries" section for gating call sites.
...
When more than one bonus damage source can trigger on the same attack, compute all of them **before** the single `Stats.take_damage()` / `DungeonFloor.show_damage()` call and add them into the base damage total — one number, one floater, one chat log line. Never call `take_damage()`/`show_damage()` once per source — besides producing a confusing multi-part log line, a source gated on `not enemy.stats.is_dead()` can silently fail to trigger if an earlier source already killed the enemy.

**Generic tooltip mechanism (add a source without touching `tooltip_formatters.gd`)**: each bonus source is appended, at the point it's computed, as `{"label": String, "color": String, "value": int}` to a local array (`bonus_sources` in `_bump_attack()`/`_resolve_cleave_attack()`, `r_bonus_sources` in `PlayerRanged.ranged_attack()`). `CombatMath.encode_bonus_sources(sources) -> String` (`scripts/entities/combat_math.gd`) joins them into the `dmg_meta` string's single `extra=` field (format: `label|color|value` per source, `;`-separated — deliberately avoids `,`/`=`, which the surrounding `key=value,key2=value2` meta string already uses). `TooltipFormatters.fmt_dmg_tooltip()` (`scripts/ui/tooltip_formatters.gd`) calls `CombatMath.decode_bonus_sources()` and renders every entry generically as `+value (label)` in the source's color. **This means adding a brand-new player damage modifier (a new talent, item effect, etc.) needs exactly one code site touched — where the bonus is computed — never a second edit in `tooltip_formatters.gd`.** The visible chat log line itself still carries no per-source text or amounts (no `(+N Frenzy)`, no God-Mode `[HP/HP]` suffix), only the combined number + damage type. See `player.gd._bump_attack()` / `PlayerRanged.ranged_attack()` (`scripts/entities/player_ranged.gd`) for the reference implementation (Rage + Frenzy + Ironwood Bark + Divine Fury summed into `bonus_dmg`/`pre_crit` before `enemy.stats.take_damage()`, each also appended to `bonus_sources`).
...
**Every `take_turn()` branch must `await` something real** (a move tween or `get_tree().create_timer(0.04 if TurnManager.fast_mode else 0.08).timeout`) — never return with zero elapsed time. `_act_toward()`'s BFS-fallback-also-fails case and the `SEARCHING`→`ROAMING` turn-exhausted transition both used to fall through with no `await` at all (a live-but-boxed-in or long-searching enemy took its "turn" instantly). This mattered beyond just visual smoothness: `TurnManager._process_enemies()` awaits every registered enemy's `take_turn()` sequentially before returning control to the player, so a zero-time turn made the round resolve instantly — indistinguishable from the genuinely-cleared-floor fast path (see `TurnManager.fast_mode` in `scripts/autoloads/CLAUDE.md`) even though an enemy was still alive and registered. Both branches now fall back to the same timer wait `_do_random_step()` already used.
...
- **Ranged ammo fired at an enemy never lands as an immediate floor pickup, hit or miss** — a non-lethal hit stays embedded (no pickup call at all) and, since the fix in `PlayerRanged.ranged_attack()`'s miss branch, a miss is treated identically (previously a miss called `PlayerAmmo.resolve_ammo_landing()` immediately, letting the player walk up and re-collect an infinite-feeling supply of arrows off every whiffed shot). The **only** way fired ammo becomes recoverable is the 50% corpse-drop roll in `_finish_kill()` when the shot that connects is the killing blow. Ammo fired at open ground/a wall (`PlayerRanged.ranged_attack_tile()`, no enemy involved) is unaffected and still lands normally.
...
- `player_throw_tool.gd` (`PlayerThrowTool`) — throw-mode and tool-priming activation, bottle fill/creation. `_throw_item`/`_tool_item` fields deliberately stay on `Player` itself (read from ~10 other input/movement call sites to cancel on move/Esc) — only the functions moved here, mutating the fields via the `player` back-reference.
...
- `combat_math.gd` (`CombatMath`, static-func-only helper, `extends RefCounted`, mirrors `scripts/ui/tooltip_formatters.gd`'s pattern) — the ADV/DISADV d20-roll resolution shared verbatim by melee/cleave/ranged (`roll_with_adv_disadv()`), weapon proficiency bonus (`weapon_prof_bonus()` — was `player.gd._weapon_prof_bonus()`, see "Weapon proficiency flags" above), `melee_reach_bonus()`, `divine_fury_flat_bonus()`, `finesse_modifier(str_mod, dex_mod, is_finesse) -> int` (returns `max(str_mod, dex_mod)` when `is_finesse`, else `str_mod` — used for both the attack roll and damage roll in `player.gd._bump_attack()` when `GameState.equipped_weapon.is_finesse`). The bonus-damage STACKING sequence itself (Frenzy/Ironwood/Divine Fury summation) and the full hit/miss/log flow stay in `player.gd._bump_attack()`/`PlayerRanged.ranged_attack()` — see "Bonus damage stacking" above.
