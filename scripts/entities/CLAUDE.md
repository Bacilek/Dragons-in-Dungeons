...
**Weapon mastery ownership**: `Stats.known_weapon_masteries: Array[String]` (default empty) + `Stats.knows_mastery(name) -> bool`. A weapon's `Item.weapon_mastery` only triggers its effect if the wielder knows that mastery — nothing currently grants entries to this array, so mastery weapons (Greataxe/Cleave, Short Bow/Vex, Handaxe/Vex, Dagger/Nick) have no mastery effect yet. See `scripts/items/CLAUDE.md`'s "Weapon masteries" section for gating call sites.
