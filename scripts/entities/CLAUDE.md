...
| `"senses": {"sight": N}` | dict | Overrides the default notice/LOS radius (`Enemy.FOV_RADIUS = 6`) via `_sight_range()`. `darkvision`/`blindsight` sub-keys are reserved, not read. |
