...
| `"thrown_weapon"` / `"unarmed_fallback"` | `{"name","dmg_min","dmg_max","damage_type","range"}` / same shape, optionally + `"attack_stat"` | Goblin Minion-only, bespoke pair (not a reusable generic mechanic — no other enemy uses either key yet). `"thrown_weapon"`: once NOT escaping (see Nimble Escape below) and the target is 2+ tiles away (not adjacent) and within `"range"` (default 4), `_decide_action()` returns a one-shot `"throw_dagger"` intent instead of the normal chase-then-melee dispatch — `_execute_dagger_throw()` resolves it via `_attack_player()`/`_attack_companion()` with `long_shot=true` forcing Disadvantage (reusing that param purely for its Disadvantage side effect, not its normal normal/long-range meaning), sets `Enemy._thrown_weapon_used = true` (a per-life one-shot flag), and registers the target with `DungeonFloor.queue_dagger_drop()`. `"unarmed_fallback"`: once `_thrown_weapon_used` is true, `_attack_target()` dispatches every subsequent attack here instead of the pool's `"multiattack"` — the Dagger is gone, thrown away. Its optional `"attack_stat"` key (Goblin Minion's Fists use `"str"` despite the Dagger being DEX-based) is a per-sub-attack override read by `Enemy._attack_bonus_for(sub)`, which every `_attack_player()`/`_attack_companion()` call now threads into `_resolve_attack_roll()`'s `attack_bonus_override` instead of always passing `-9999`/falling back to the enemy-wide `attack_profile.attack_stat` default — falls back to the normal `_attack_bonus()` for every pre-existing multiattack/ability entry that doesn't set it. **Recovering the thrown Dagger**: `DungeonFloor._pending_dagger_drops`/`_resolve_pending_dagger_drops()` (`scripts/world/CLAUDE.md`) — a 50% chance, checked once on the player's next real turn after the goblin dies (`Enemy.die()` queues it, `TurnManager.player_turn_started` resolves it), to drop a normal pickupable Dagger `Item` at wherever the throw's target currently stands. Unconditional on the throw actually having landed — `_attack_player()`/`_attack_companion()` don't return their hit result, so gating strictly on a hit would need a broader refactor; documented simplification for this one enemy's flavor mechanic. |
