...
every save keys off it) is the real 5e Bearded Devil stat
...
Tier 1 (levels 1–7): earns 6 talent points, spent across 3 talents — Psycho, Bruiser, Battlefield Expert (max 3 ranks each = 9 total possible cost; the rule doc's 4th talent per tier is not authored yet). Points are granted on the level-up transitions into 2-7 (`GameState.TIER_LEVEL_RANGES[1] = [1, 7]`) — level 1 itself grants nothing (no level-up fires). These base talents (added per `markdowns/barbarian_base.md`) are shared by every subclass and grant no ability-bar entry — they're pure passive/reactive hooks read directly via `GameState.get_talent_rank()` at point of use. (Reckless Attack and Danger Sense were removed — vestigial, unused talents; nothing reads their old talent ids anymore.) Starting equipment given in `GameState.give_class_starting_items()` → `_give_barbarian_starting_items()`:
...
**Barbarian Tier 2 subclasses** (points at levels 8–13; subclass pick opens the moment level 8 is reached — no boss kill). Full source specs (superseding the earlier `barbarian-tier1-rework-v2-prompt.md` design): `markdowns/barbarian_base.md`, `markdowns/berserker.md`, `markdowns/scarred_warrior.md`, `markdowns/wild_heart.md`, `markdowns/zealot.md`.
- Level-point schedule (`GameState.TIER_LEVEL_RANGES`, `docs/architecture/class-progression-rules.md` §4): levels 1–7 → Tier 1 (6 points), 8–13 → Tier 2 (6), 14–19 → Tier 3 (6, no content yet), 20 → Tier 4 / Epic Boon (1, no content yet). Level cap `Stats.MAX_LEVEL` = 20.
- `GameState.tier2_unlocked: bool` — set by `unlock_tier2()` (level-gated via `_check_tier2_level_gate()` — see `scripts/autoloads/CLAUDE.md`). `_setup_barbarian_tier2_talents()` appends 3 `Talent` objects to `_class_talents`.
...
genuinely has none). **Prepared count**: `SpellcasterState.prepared_max()` caps by role — Ranger
(HALF_CASTER) uses the real half-caster formula `WIS mod + character_level / 2` clamped to 1..3,
full casters use `min(character_level, 5)` (class-progression-rules.md §6.2).
...
branch — Warlock is FULL_CASTER in `Stats.CLASS_ROLE`, so it gets `min(character_level, 5)`.
...
- **`SpellcasterState.prepared_spells`/`prepared_max(stats)`** — prepared count is
  `min(character_level, 5)` for a full caster (cantrips never count against it) — capped by role
  per class-progression-rules.md §6.2, since a prepared spell occupies an ability-bar slot. `known_spells` holds BOTH cantrips and leveled spells now
