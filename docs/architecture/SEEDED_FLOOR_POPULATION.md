...
**Scope note — this narrows SAVE_LOAD_ARCHITECTURE.md §6.** The project owner has ruled multiplayer out as a design driver (it may never exist). Consequently §6's project-wide `Rng` autoload service, the retrofit of combat-roll RNG (`randi_range` in `player.gd`/`enemy.gd`/`stats.gd`), and lockstep concerns (stable Dictionary iteration for sync, replay verification) are **over-scoped for this project's actual needs and should be treated as dormant** — do not schedule the "project-wide Rng retrofit" session from `IMPLEMENTATION_SEQUENCE.md` item 7 unless multiplayer is ever revisited. Combat rolls, status-effect rolls, enemy AI decision rolls (`resist_check`, roam shuffles, random steps), trap-trigger checks, boss-loot rolls at kill time, and `resolve_push()` wall damage all stay on the global unseeded RNG **indefinitely**. Only *floor-load-time population* needs determinism, and this doc is its complete spec.
...
Explicitly **NOT** converted (runtime, not load-time — reconfirmed against the file): the trap-trigger DEX-check dice (~855), `_roll_boss_loot_item()` (~1466, rolled at boss-kill time), `resolve_push()` wall-slam damage (~1512), and the trap reveal/disarm paths. Also not converted: everything in `player.gd`, `enemy.gd`, `companion.gd`, `stats.gd`.
